...
//    ImVec4 clear_color = ImVec4(0.45f, 0.55f, 0.60f, 1.00f);
    ...
    bool ret = LoadTextureFromFile("../Images/plus_sign.png", &my_image_texture, &my_image_width, &my_image_height);
    IM_ASSERT(ret);

    int my_image_width2 = 0;
    int my_image_height2 = 0;
    GLuint my_image_texture2 = 0;
    bool ret2 = LoadTextureFromFile("../Images/join.png", &my_image_texture2, &my_image_width2, &my_image_height2);
    IM_ASSERT(ret2);
...
//            //ImGuiWindowFlags_NoTitleBar
            ...
            if (first_use) {
                ...
                if (ImGui::BeginPopupModal("CREATE SCHOOL", NULL))
                {
                    ...
                    static char buf3[64] = ""; ImGui::InputText("##a", buf3, 64);// Display some text (you can use a format strings too)
                    ImGui::Text("Password");
                    static char password2[64] = ""; ImGui::InputText("##b", password2, IM_ARRAYSIZE(password2), pwflags2);
                    ImGui::SameLine();
                    ImGui::Checkbox("Show Password", &showPW2);
                    ImGui::Text("Confirm Password");
                    static char password3[64] = ""; ImGui::InputText("##c", password3, IM_ARRAYSIZE(password3), pwflags3);
                    ImGui::SameLine();
                ...
                ImGui::SetCursorPos(ImVec2((ImGui::GetWindowSize().x - (my_image_width2 / 15) - (1.5 * my_image_width / 5)) * 0.5f,(ImGui::GetWindowSize().y - (my_image_height2 / 15) -((my_image_height2 / 15) - my_image_height / 5)) * 0.5f));
                if (ImGui::ImageButton((void *) (intptr_t) my_image_texture2,ImVec2(my_image_width2 / 15, my_image_height2 / 15))) {
...
//                        active_tabs.push_back(to_string(next_tab_id++)); // Add new tab (action that h)
                        //TODO: change to push back courses and asking for information on courses after merging files
                    }
...
//                        active_tabs.push_back(next_tab_id++); // Add new tab
                    ...
                    for (int n = 0; n < active_tabs.Size;) {
                        bool open = true;
                        char name[16];
                        snprintf(name, IM_ARRAYSIZE(name), "%s", active_tabs[n].c_str());
                        if (ImGui::BeginTabItem(name, &open, ImGuiTabItemFlags_None)) {
                            if (ImGui::BeginTable(
                                    fmt::format("Students of {}", active_tabs[n]).c_str(), 6,
                                ...
                                ImGui::TableNextColumn();
                                ImGui::Text("Address");
                                for (auto student : db.getCourses()[active_tabs[n]].getStudents()) {
                                    ImGui::TableNextColumn();
                                    ...
                                    ImGui::TableNextColumn();
                                    ImGui::Text(to_string(student->getGrade()).c_str());
                                    ImGui::TableNextColumn();
                                    ImGui::Text(to_string(student->getNumLates()).c_str());
                                    ImGui::TableNextColumn();
...
//                            cout << endl;
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
            ...
            {
                ImGui::Text("YOUR ACCOUNT HAS BEEN SUCCESSFULLY CREATED. \nLOG IN THROUGH THE LOG IN WINDOW!");
